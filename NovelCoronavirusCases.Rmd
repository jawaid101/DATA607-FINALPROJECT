...
seq_id_all <- seq(from = start_href, length.out = max_hrefs)
glimpse(seq_id_all)
```

Process URLs using cluster (**map** step of map/reduce))
```{r par-process}
work_result <- Cluster.ParApply(cr, seq_id_all, ScrapeStats_USA)
```
...
Combine nested results (**reduce** step of map/reduce)
