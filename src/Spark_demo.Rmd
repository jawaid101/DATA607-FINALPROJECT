...
if (is_global_data) {
    df1 <- df |>
        ...
        unique()  # keep one unique max death row       
} else {
    df1 <- df |>
        filter(!is.na(Lat)) |>
        filter(!is.na(Long_)) |>
...
df1 <-
    df1 |> mutate(scaled_tot_deaths = Deaths / max_deaths, .after = Deaths)
...
if (is_global_data) {
    m <- leaflet::leaflet(df1) %>%
        ...
        leaflet::addCircleMarkers(
            lng = ~ Long_,
            lat = ~ Lat,
            label = ~ paste0(Country_Region, ", Deaths: ", Deaths),
            color = 'red',
