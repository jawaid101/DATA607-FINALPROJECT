...
```{r}
# plot data
df_quarter <- Truncate(df, keep_type = '3 month', is_global_data)
df_quarter

rnd_locations <- sample(df$location, 20)

# load ggplot2
library(hrbrthemes)

# A basic scatterplot with color depending on Species
p <- df_latest |> #filter(location %in% rnd_locations) |>
    ggplot(aes(x=gdp_per_capita, y=total_deaths_per_million, color=location)) + 
    geom_point() +
    theme_ipsum()
ggplotly(p)
```
